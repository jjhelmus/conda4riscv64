...
sync_recipe() {
	...
	cd ${feedstock_dir}
	git checkout main ${GIT_ARGS}
	git fetch origin ${GIT_ARGS}
	git reset --hard origin/main ${GIT_ARGS}

	if [ ! -d ${RECIPE_DIR}/${feedstock_dir}/recipe ]; then
		echo "[INFO] Directory for recipe ${recipe} doesn't exist. Creating. "
		mkdir -p ${RECIPE_DIR}/${feedstock_dir}/recipe
	fi
	echo "[INFO] Copying recipe to ${RECIPE_DIR}/${recipe}/recipe"
	cp -r recipe/* ${RECIPE_DIR}/${feedstock_dir}/recipe/
